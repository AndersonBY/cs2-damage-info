...
if [[ "${tag%%-mbsifu.*}" != "v5.0.1" ]]; then
  echo "release tag ${tag} does not match project version v5.0.1" >&2
  exit 2
fi
...
"${dotnet}" build "${repo_root}/src/HybridDamageInfo.csproj" -c Release --nologo
plugin_dir="${staging}/addons/counterstrikesharp/plugins/HybridDamageInfo"
mkdir -p "${plugin_dir}"
install -m 0644 "${repo_root}/src/bin/Release/net10.0/HybridDamageInfo.dll" "${plugin_dir}/HybridDamageInfo.dll"
install -m 0644 "${repo_root}/src/bin/Release/net10.0/HybridDamageInfo.deps.json" "${plugin_dir}/HybridDamageInfo.deps.json"
...
archive="${output_dir}/cs2-damage-info-${tag}-linux-x64.zip"
